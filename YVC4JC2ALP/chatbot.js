...
'<input type="text" id="openai-chat-input" placeholder='+'"' + 'Geben Sie Ihre Frage ein...' +'"'+ '>' +
...
addChatbotMessage("Hallo! Ich bin der Chatbot. Womit kann ich Ihnen behilflich sein? (Die erste Nachricht kann einige Sekunden dauern)");
...
jQuery(document).ready(function($) {
    ...
    async function handleQuestion(questionElement, answerElement) {
        ...
        $.get('https://raw.githubusercontent.com/medanticancerbelievescotland/knowledge/main/knowledge_de.txt')
            .then(function(data) {
                var knowledge = data;    
                    ...
                    $.ajax({
                        ...
                        beforeSend: function(xhr) {
                            ...
                            $(answerElement).append('<div class="loading">Antwort abrufen...</div>');
                        },
                        success: function(response) {
                            $('.loading').remove();
                            if (response.choices && response.choices[0] && response.choices[0].text) {
                                $(answerElement).append('<div class="question-answer"><strong>Fragen:</strong> ' + question + '<br><strong>Antworten:</strong> ' + response.choices[0].text + '</div><hr>');
                                ...
                                $(answerElement).append('<div class="error"><strong>Error:</strong> Antwort konnte nicht abgerufen werden.</div><hr>');
                            }
                        },
                        error: function() {
                            $('.loading').remove();
